...
<script type="text/javascript">
	...
	$(document).ready(function() {	
		...
		$("#plus").click(function(event) {
			...
			//$("#temporal-"+count+" > #select-9999").attr("id", "select-"+count);
			newForm.children( "#select-9999" ).attr("id", "select-"+count);
			...
			updateFields(idNewForm, count, true);
		 ...
		 */
		function updateFields(idNewForm, index, cond) {
		...
		function changeForm(select) {
			...
			$("#temporal-"+index+" .typeForm").fadeOut(function() {
				...
				$("#temporal-"+index+" .typeForm").fadeIn(function() {
					updateFields("temporal-"+index, index, false);
				});
			});						
		}
		...
		function removeTemporal(event) {
			...
			$('#temporal-'+index).slideUp("slow", function() {
				...
				$("#temporals .tempo").each(function(index) { 
					updateFields($(this).attr("id"), index, false);
...
<h2 class="subTitle"><@s.text name='manage.metadata.tempcoverage.title'/></h2>
	...
	<div id="temporals">
		...
		<#list eml.temporalCoverages as temporalCoverage>
			...
			<div id="select-${temporalCoverage_index}">
				<@select i18nkey="eml.temporalCoverages.type"  name="tempTypes-${temporalCoverage_index}" options=tempTypes value="${temporalCoverage.type}" help="i18n"/>
			</div>
			<!-- Adding new subform -->
...
<div id="temporal-99999" class="tempo clearfix" style="display:none">
	...
	<div id="select-9999">
		<@select i18nkey="eml.temporalCoverages.type"  name="tempTypes" options=tempTypes help="i18n" />  
	</div>
</div>
